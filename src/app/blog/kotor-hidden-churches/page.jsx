import { t, buildAlternates } from '../../metadata';
import KotorHiddenChurches from '@/src/components/pages/blog/KotorHiddenChurches';

export async function generateMetadata() {
  const title = t('en', 'blogHiddenChurches.title') + ' | Kotor Car Rental';
  const description = t('en', 'blogHiddenChurches.description');
  return {
    title: title,
    description: description,
    alternates: buildAlternates('blog/kotor-hidden-churches'),
    openGraph: { title, description, type: 'website' },
  };
}

export default function KotorHiddenChurchesRoute() {
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": t('en', 'blogHiddenChurches.title'),
    "description": t('en', 'blogHiddenChurches.description'),
    "image": "https://www.kotorcarrental.com/img/blog-kotor-walls.webp",
    "datePublished": "2026-04-09",
    "dateModified": "2026-04-09",
    "author": { "@type": "Organization", "name": "Kotor Car Rental", "url": "https://www.kotorcarrental.com" },
    "publisher": { "@type": "Organization", "name": "Kotor Car Rental", "url": "https://www.kotorcarrental.com" }
  };
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema) }} />
      <KotorHiddenChurches />
    </>
  );
}
