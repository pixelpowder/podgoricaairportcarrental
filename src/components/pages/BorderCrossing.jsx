...
export default function BorderCrossing() {
  ...
  return (
    <ContentPage title={t("border-crossing.title")} subtitle={t("border-crossing.subtitle")} description={t("border-crossing.seoDesc")} image="/img/montenegro-coast-road.webp">
      ...
      <p>{t('borderCrossingBody.croatiaFerry')} {t('borderCrossingBody.croatiaFerryLink').split('{{link}}')[0]} <a href={localePath('/dubrovnik-airport')}>{t('borderCrossingBody.dubrovnikAirportLink')}</a> {t('borderCrossingBody.croatiaFerryLink').split('{{link}}')[1]}</p>
      ...
      <h2>{t('borderCrossingBody.returnTitle')}</h2>
      <p>{t('borderCrossingBody.returnP')}</p>
